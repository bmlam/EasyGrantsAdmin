--@/tmp/logging/loggingInOracleDB/log_table.sql
--@/tmp/logging/loggingInOracleDB/debug_user.sql
--@/tmp/logging/loggingInOracleDB/long_log.sql
--@/tmp/logging/loggingInOracleDB/pck_std_log-def.sql
--@/tmp/logging/loggingInOracleDB/pck_std_log-impl.sql
--@/tmp/logging/loggingInOracleDB/pck_std_log-shorthands.sql
...
select component loc, 'Y'||to_char(timestamp, 'rr.mmdd hh24:miss')ts, text txt, t.* from log_table t order by id desc
;
...
order by 1
;
-- view to be owned by SYS
create or replace view sys.all_grantees
as
select username grantee from dba_users
union 
...
, 'Y'  grantee_is_regexp, 'SYNONYM' privilege, 'N' grantable
from dual
) src
